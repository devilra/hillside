...
app.use((req, res, next) => {
  if (dbError) {
    return res.status(500).json({
      ...
      message:
        "The Hillside backend server is running, but failed to connect to the MySQL database.",
      error: dbError.message,
      suggestion:
        "Please verify that the database user has been added to the database with all privileges in cPanel MySQL Databases page, and that DB_HOST, DB_NAME, DB_USER, and DB_PASSWORD are correct.",
    });
  }
  next();
});
...
app.use(
  "/hillsite-api/uploads",
  express.static(path.join(__dirname, "uploads")),
);
app.use("/uploads", express.static(path.join(__dirname, "uploads")));

// Serve uploaded videos (Local Storage)
app.use(
  "/videos",
...
const healthCheckHandler = (req, res) => {
  res.json({
    status: "OK",
    message: "Hillside Lead Portal Backend API is running.",
  });
};
...
const startServer = async () => {
  try {
    ...
    console.error(
      "[HILLSIDE SERVER] Database connection failed during startup:",
      error,
    );
    dbError = error;
